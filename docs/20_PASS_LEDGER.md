...
| 4 | `INSTALL.md` | Missing `claudecode` in platform matrix. Manual install examples used old `caveman` names. | Added `claudecode` row. Fixed manual install examples to use `signalmode`. | File read verified. |
...
| 7 | `package.json`, `README.md` | Unsupported "65-75% token savings" benchmark claim. | Removed the unsupported claim from both files. | File read verified. |
